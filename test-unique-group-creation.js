// Test unique group creation with Pinata API
const PINATA_JWT = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..fND1AtRjFCAx7KspftaxcWjr4b40mSGQA5qpf5fRHYw';

async function testUniqueGroupCreation() {
  console.log('🧪 Testing unique group creation with Pinata API...');
  
  // Generate unique group name
  const timestamp = Date.now();
  const uniqueGroupName = `addr_17dc79c8_rice_basmati_${timestamp}`;
  console.log('🔍 Testing unique group name:', uniqueGroupName);
  console.log('🔍 Length:', uniqueGroupName.length);
  console.log('🔍 Valid characters only?', /^[a-z0-9_]+$/.test(uniqueGroupName));
  
  try {
    const payload = JSON.stringify({ name: uniqueGroupName });
    console.log('🔍 Payload:', payload);
    
    const response = await fetch("https://api.pinata.cloud/v3/groups/public", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${PINATA_JWT}`,
      },
      body: payload,
    });

    console.log('Response status:', response.status);
    const responseText = await response.text();
    console.log('Response:', responseText);

    if (response.ok) {
      const data = JSON.parse(responseText);
      console.log('✅ SUCCESS! Unique group created:', data);
      console.log('✅ Group ID:', data.data.id);
    } else {
      const errorData = JSON.parse(responseText);
      console.log('❌ FAILED:', errorData);
    }

  } catch (error) {
    console.error('❌ ERROR:', error);
  }
}

// Run the test
testUniqueGroupCreation();
