-- Fix duplicate function names by dropping existing functions first
-- Run this in your Supabase SQL Editor

-- Drop existing functions if they exist
DROP FUNCTION IF EXISTS add_to_marketplace(UUID, UUID, TEXT, NUMERIC, NUMERIC);
DROP FUNCTION IF EXISTS add_to_marketplace(UUID, UUID, TEXT, NUMERIC, NUMERIC, TEXT);
DROP FUNCTION IF EXISTS purchase_from_marketplace(BIGINT, NUMERIC);
DROP FUNCTION IF EXISTS purchase_from_marketplace(BIGINT, NUMERIC, TEXT);

-- Also drop any functions with different parameter types
DROP FUNCTION IF EXISTS add_to_marketplace CASCADE;
DROP FUNCTION IF EXISTS purchase_from_marketplace CASCADE;

-- Now create the marketplace table (drop if exists first)
...
-- Create RPC functions for marketplace operations (with unique names)
...
GRANT EXECUTE ON FUNCTION add_to_marketplace(UUID, UUID, TEXT, NUMERIC, NUMERIC) TO authenticated;
GRANT EXECUTE ON FUNCTION purchase_from_marketplace(BIGINT, NUMERIC) TO authenticated;

-- Check the final structure
SELECT 'SUCCESS: Marketplace table and RPC functions created successfully!' as result;
