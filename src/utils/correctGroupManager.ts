...
/**
 * Correct Group Manager
 * Uses the correct Pinata API with JWT authentication and proper endpoints
 */
export class CorrectGroupManager {
  private static instance: CorrectGroupManager;

  private constructor() {}

  public static getInstance(): CorrectGroupManager {
    if (!CorrectGroupManager.instance) {
      CorrectGroupManager.instance = new CorrectGroupManager();
    }
    return CorrectGroupManager.instance;
  }
  ...
  public async createGroup(groupName: string): Promise<string> {
    try {
      const response = await fetch('https://api.pinata.cloud/pinning/createGroup', {
        method: 'POST',
        headers: {
      ...
      console.log(`Created Pinata group: ${groupName} with ID: ${data.id}`);
      return data.id;
    } catch (error) {
...
export const correctGroupManager = CorrectGroupManager.getInstance();
