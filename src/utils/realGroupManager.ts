...
/**
 * Real Group Manager
 * Actually creates Pinata groups and uploads files to them
 */
export class RealGroupManager {
  private static instance: RealGroupManager;

  private constructor() {}

  public static getInstance(): RealGroupManager {
    if (!RealGroupManager.instance) {
      RealGroupManager.instance = new RealGroupManager();
    }
    return RealGroupManager.instance;
  }
  ...
  /**
   * Create a new Pinata group
   */
  public async createGroup(groupName: string): Promise<string> {
    try {
      const response = await axios.post(
        'https://api.pinata.cloud/pinning/createGroup',
        { name: groupName },
        {
          headers: {
  ...
  /**
   * Upload file to a specific Pinata group
   */
...
export const realGroupManager = RealGroupManager.getInstance();
