 ...
 * Uses metadata-based grouping since Pinata groups API might not be available
 */
export class WorkingGroupManager {
  private static instance: WorkingGroupManager;

  private constructor() {}

  public static getInstance(): WorkingGroupManager {
    if (!WorkingGroupManager.instance) {
      WorkingGroupManager.instance = new WorkingGroupManager();
    }
    return WorkingGroupManager.instance;
  }
  ...
  /**
   * Upload file with group metadata
   */
  ...
  public async getGroupFiles(groupId: string): Promise<any[]> {
    try {
      const response = await axios.get(
        `https://api.pinata.cloud/data/pinList?metadata[keyvalues]={"groupId":{"value":"${groupId}","op":"eq"}}`,
        {
          headers: {
            'pinata_api_key': PINATA_CONFIG.apiKey,
            'pinata_secret_api_key': PINATA_CONFIG.apiSecret,
...
export const workingGroupManager = WorkingGroupManager.getInstance();