...
/**
 * Super Simple Group Manager
 * Uses the existing working IPFS methods with minimal changes
 */
export class SuperSimpleGroupManager {
  private static instance: SuperSimpleGroupManager;
  ...
  public static getInstance(): SuperSimpleGroupManager {
    if (!SuperSimpleGroupManager.instance) {
      SuperSimpleGroupManager.instance = new SuperSimpleGroupManager();
    }
    return SuperSimpleGroupManager.instance;
  }
  ...
  /**
   * Upload harvest certificate using existing working method
   */
  public async uploadHarvestCertificate(
    ...
    try {
      ...
      // Use existing working uploadCertificate method
      const batchIdNum = parseInt(batchData.batchId);
      const batchDataForUpload = {
      ...
      const ipfsHash = await this.ipfsService.uploadCertificate(pdfBlob, batchIdNum, batchDataForUpload);
      
      console.log(`Uploaded harvest certificate for batch ${batchData.batchId}, Group ID: ${groupId}, IPFS: ${ipfsHash}`);
  ...
  /**
   * Upload purchase certificate using existing working method
   */
  public async uploadPurchaseCertificate(
    ...
    try {
      ...
      // Use existing working uploadCertificate method
      const batchIdNum = parseInt(purchaseData.batchId);
      const batchDataForUpload = {
        crop: purchaseData.productDetails.cropType,
        variety: purchaseData.productDetails.variety,
        farmer: purchaseData.from,
        groupId: groupId,
...
export const superSimpleGroupManager = SuperSimpleGroupManager.getInstance();
