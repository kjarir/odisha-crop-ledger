...
/**
 * Final Group Manager
 * Uses the working Pinata file upload API with metadata grouping
 */
export class FinalGroupManager {
  private static instance: FinalGroupManager;

  private constructor() {}

  public static getInstance(): FinalGroupManager {
    if (!FinalGroupManager.instance) {
      FinalGroupManager.instance = new FinalGroupManager();
    }
    return FinalGroupManager.instance;
  }

  /**
   * Generate a group ID based on owner and crop
   */
  public generateGroupId(farmerName: string, cropType: string, variety: string): string {
    const cleanFarmerName = farmerName.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
    const cleanCropType = cropType.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
    const cleanVariety = variety.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
    const timestamp = Date.now();
    
    return `group-${cleanFarmerName}-${cleanCropType}-${cleanVariety}-${timestamp}`;
  }

  /**
   * Generate a group name for display
   */
  public generateGroupName(farmerName: string, cropType: string, variety: string): string {
    return `${farmerName} - ${cropType} ${variety}`;
  }

  /**
   * Upload file with group metadata using working Pinata API
   */
  public async uploadFileWithGroupMetadata(
    ...
    try {
      ...
      console.log(`Uploaded file ${fileName} with group metadata, Group: ${groupName}, IPFS: ${ipfsHash}`);
      return ipfsHash;
    } catch (error) {
      console.error('Error uploading file with group metadata:', error);
      throw new Error('Failed to upload file with group metadata');
    }
  }
  ...
  public async uploadHarvestCertificate(
    ...
    try {
      ...
      const groupId = this.generateGroupId(batchData.farmerName, batchData.cropType, batchData.variety);
      ...
      const ipfsHash = await this.uploadFileWithGroupMetadata(pdfBlob, fileName, groupId, groupName, metadata);
  ...
  /**
   * Get all files in a group by searching metadata
   */
  public async getGroupFiles(groupId: string): Promise<any[]> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to get group files: ${response.status} ${response.statusText}`);
      }
...
export const finalGroupManager = FinalGroupManager.getInstance();
